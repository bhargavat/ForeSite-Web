...
export class EventComponent implements OnInit {
  ...
  getEventDetails(id:any): void{
    ...
    this.eventService.getEventDetails(query)
    .subscribe(response => {
  ...
  collect_response(info:any, response:any){
    ...
    info["start_date_time_formatted"] = this.parse(response["start_date"], response["start_time"]);
  ...
  parse(start_date: any, start_time: any){
    ...
    if (date.getDate() % 10 == 1 && Math.floor(date.getDate() / 10) != 1) {
      return formatDate(date, "EEEE dd", 'en-US') + "st" + 
      formatDate(date, " MMMM yyyy hh:mma", 'en-US');
    }else if (date.getDate() % 10 == 2 && Math.floor(date.getDate() / 10) != 1) {
      return formatDate(date, "EEEE dd", 'en-US') + "nd" + 
      formatDate(date, " MMMM yyyy hh:mma", 'en-US');
    }else if (date.getDate() % 10 == 3 && Math.floor(date.getDate() / 10) != 1) {
      return formatDate(date, "EEEE dd", 'en-US') + "rd" + 
      formatDate(date, " MMMM yyyy hh:mma", 'en-US');
    }else {
      return formatDate(date, "EEEE dd", 'en-US') + "th" + 
      formatDate(date, " MMMM yyyy hh:mma", 'en-US');
    }
  }
